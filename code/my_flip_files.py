...
def check_hero_pixel(filename, max_pixels):
    s = hero_pixel_number(filename)
    if s >= 1 and s <= max_pixels:
#        print(s)
         return True
    return False
...
if __name__ == '__main__':
    ...
    parser.add_argument('-maxpixel', "--maxpixel", default=400, type=int)
...
#    print(images_dir)
    ...
    for f in files:
        valid_flip = check_hero_pixel(f, max_pixels)
        if valid_flip:
#            print(f)
...
#            print(filename)
            ...
            if img_filename != '':
                flip_and_save_images(mask_dir, filename + '.png', check_only)
#                    print(image_filename)
                flip_and_save_images(images_dir, img_filename + '.jpeg', check_only)
                flipped_file_count += 1

    print('Total flipped file number = %d for images and mask each.' % flipped_file_count)
